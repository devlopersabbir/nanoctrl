...
static bool cli_approval(nano_session_t *s, const char *remote_ip, void *user_data) {
    ...
    printf("A controller at %s wants to control this computer.\n", remote_ip ? remote_ip : "unknown");
...
static int run_cli_controller(const char *host_str, const char *pin) {
    signal(SIGINT, sigint_handler);
    signal(SIGTERM, sigint_handler);
    ...
    char tmp[128];
    strncpy(tmp, host_str, sizeof(tmp) - 1);
    ...
    printf("[NANOCTRL] Connecting to %s:%u with PIN %.3s %.3s...\n", host, port, pin, pin + 3);
...
int main(int argc, char *argv[]) {
    ...
    if (is_host) {
        if (cli_mode) {
            return run_cli_host(port, custom_pin);
        } else {
    ...
    if (is_controller && target && custom_pin) {
        if (cli_mode) {
            return run_cli_controller(target, custom_pin);
        } else {
            char host[128] = "127.0.0.1";
